...
namespace Simulator.PhysicalModeling
{
    class GameBall : DynamicObject
    {
        ...
        public void Update(float dt, BoundingBox robotBox, BoundingSphere robotSphere, Vector3 robotVelocity, float angularVelocity)
        {
            if (ballPossessed)
            {
                ...
                Position = robotSphere.Center / FieldConstants.PIXELS_IN_ONE_METER + Vector3.UnitY * BALL_RADIUS;
                Velocity = Vector3.Zero;
                return;
            }

            bool robotIntersection = false;
            ...
            else
            {
                if (robotIntersection)
                {
                    ...
                    if (robotBox.Intersects(new Ray((Position - Vector3.UnitY * BALL_RADIUS * 0.5f) * FieldConstants.PIXELS_IN_ONE_METER, Velocity)) != null)
                    {
                        if (ballInAir)
                            ballPossessed = true;
                        else
            ...
            //intersections
            if (!wallIntersection && (Position.X < BALL_RADIUS || Position.X + BALL_RADIUS > FieldConstants.WIDTH_IN_METERS))
            {
                Velocity -= new Vector3(1.8f * Velocity.X, Velocity.Y / 10, Velocity.Z / 10);
                wallIntersection = true;
            }
            else if (!wallIntersection && (ballFront.Z < 0 || ballFront.Z > FieldConstants.C * FieldConstants.HEIGHT))
            {
            ...
            else if (!wallIntersection && (ballInAir && Velocity.Y < 0 && Position.Y < BALL_RADIUS))
            {
                ...
                wallIntersection = false;
            Field.UpdateTrussBallInteraction(dt, this);
        ...
        {
            return new BoundingSphere(Position * FieldConstants.PIXELS_IN_ONE_METER,
                    BALL_RADIUS * FieldConstants.PIXELS_IN_ONE_METER);
        }
